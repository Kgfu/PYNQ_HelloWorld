...
void colordetect_accel(stream_t& src,
    ...
    #pragma HLS INTERFACE m_axi      port=low_thresh    offset=slave  bundle=gmem0 depth=__XF_DEPTH_FILTER
    #pragma HLS INTERFACE m_axi      port=high_thresh   offset=slave  bundle=gmem1 depth=__XF_DEPTH_FILTER
...
//Willy debug - s
    #pragma HLS INTERFACE s_axilite port=t1
    #pragma HLS INTERFACE s_axilite port=t2
//Willy debug - e
	...
	unsigned char low_t[9];
	unsigned char high_t[9];	
	int index = 0;
	for (index = 0; index < 9; index++)
	{
		low[index] = low_thresh[index];
		low_t[index] = low_thresh[index];
		high[index] = high_thresh[index];
		high_t[index] = high_thresh[index];
	}
   ...
   // Do the color thresholding:
    ...
    xf::cv::colorthresholding<IN_TYPE, OUT_TYPE, MAXCOLORS, HEIGHT, WIDTH, NPIX>(rgb2hsv, imgHelper1, low, high);
	 									    
//Willy debug - s
//Add debug code to verify m_axi parameter
*t2 = 0;
if(t1 < 9)
{
  //low_thresh
...
//    xf::cv::resize<INTERPOLATION, TYPE, HEIGHT, WIDTH, HEIGHT, WIDTH, NPIX, MAXDOWNSCALE>(src_mat, dst_mat);
    ...
    //xfMat2axis<DATA_WIDTH, IN_TYPE, HEIGHT, WIDTH, NPIX>(imgOutput, dst);
    xfMat2axis<DATA_WIDTH, OUT_TYPE, HEIGHT, WIDTH, NPIX>(imgOutput, dst);
//    xfMat2axis<DATA_WIDTH, IN_TYPE, HEIGHT, WIDTH, NPIX>(rgb2hsv, dst);
//    xfMat2axis<DATA_WIDTH, IN_TYPE, HEIGHT, WIDTH, NPIX>(rgb2hsv, dst);

//    xfMat2axis<DATA_WIDTH, OUT_TYPE, HEIGHT, WIDTH, NPIX>(imgHelper1, dst);
